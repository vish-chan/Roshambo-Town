...
class World extends Component {
    ...
    handleTouchStart(event) {
        if(!this.state.showExitModal) {
            event.preventDefault();
            this.touchstartX = this.touchendX = event.touches[0].clientX;
            this.touchstartY = this.touchendY = event.touches[0].clientY;
            this.timeout = setTimeout(this.setModalState, 500);
        }
    }

    handleTouchMove(event) {
        if(this.timeout!==null && !this.state.showExitModal) {
            event.preventDefault();
    ...
    render() {
        ...
        return(
            <div ref={world => this.world = world} id="world" style={style}>
                    ...
                    <ConversationDisplay />
                    <Inventory />
                    <CustomModal show={this.state.showExitModal} speedChange={this.props.handlePlayerSpeedChange} confirmLink={this.props.handleBack} cancelLink={this.resetModalState} />
            </div>
        );
        }
    }
